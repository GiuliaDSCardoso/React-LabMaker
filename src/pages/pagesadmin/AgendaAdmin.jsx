...
function aplicarFiltro(lista, filtro) {
  ...
  return (
    ...
    <div className="flex flex-col justify-center items-center p-6 w-full  space-y-6">
      ...
      <h2 className="text-xl md:text-2xl font-bold">Solicitações / Ocupações</h2>
      <div className="flex gap-4 mb-4">
      <button
        onClick={() => setFiltro("recentes")}
        className={`px-4 py-2 text-lg md:text-xl rounded ${
          filtro === "recentes" ? "bg-blue-600 text-white" : "bg-gray-200"
        }`}
      >
        Mais recentes
      </button>
      ...
      >
        Esta semana
          ...
          <div
            ...
            `}
          >
            <p className="text-lg md:text-xl"><b>Data:</b> {item.data}</p>
            ...
            <p className="text-lg md:text-xl"><b>Motivo:</b> {item.motivo}</p>
            ...
            {item.tipo === "USUARIO" && (
              <>
                <p className="text-lg md:text-xl"><b>Solicitante:</b> {item.historico?.nome}</p>
                <p className="text-lg md:text-xl"><b>Email:</b> {item.historico?.email}</p>

                <a
                  href={`https://wa.me/55${item.historico?.telefone?.replace(/\D/g, "")}`}
                  target="_blank"
